...
function Home() {
    ...
    const [publications, setPublications] = useState([]); // Теперь храним текущие публикации
    const [filteredPublications, setFilteredPublications] = useState([]); // Фильтрованные публикации
    const [error, setError] = useState('');
    const [success, setSuccess] = useState('');
    const [openError, setOpenError] = useState(false);
    const [openSuccess, setOpenSuccess] = useState(false);
    const [currentPage, setCurrentPage] = useState(1);
    const [totalPages, setTotalPages] = useState(1); // Общее количество страниц от сервера
    ...
    useEffect(() => {
        const fetchPublishedPublications = async (page = 1) => {
            try {
                ...
                console.log('Server response for public publications:', response.data); // Отладочный лог
                setPublications(response.data.publications);
                setFilteredPublications(response.data.publications); // Изначально отображаем полученные данные
                setTotalPages(response.data.pages || 1); // Устанавливаем общее количество страниц
            } catch (err) {
        ...
        fetchPublishedPublications(currentPage);
    }, [currentPage]); // Обновляем при изменении страницы
    ...
    }, [searchTerm, filterType, filterYear, publications]);

    const currentPublications = filteredPublicationsMemo; // Используем filteredPublicationsMemo напрямую, полагаясь на серверную сортировку
